...
class CatVsDogsClassifier(object):
    def __init__(self, is_model_heavy=True):
        ...
        train_dataset = TensorDataset(torch.FloatTensor(Xtrain), torch.tensor(Ytrain, dtype=torch.int64))
        test_dataset = TensorDataset(torch.FloatTensor(Xtest), torch.tensor(Ytest, dtype=torch.int64))
